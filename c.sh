...
for minimum in 5; do # 10 20; do
  for model in vit_large; do # do mobilenetv2 resnet50v2; do
    for dataset in pr; do
      for size in 512; do # 400 512; do
        for color in RGB; do # RGB; do
          for classifier in SVC; do # RandomForestClassifier KNeighborsClassifier MLPClassifier SVC; do #DecisionTreeClassifier RandomForestClassifier KNeighborsClassifier MLPClassifier SVC
            python main.py -i /home/msechineli/Documentos/TCC/features/${dataset}+${minimum}/${color}/${size}/${model} -c ${classifier} -o /home/msechineli/Documentos/TCC/smote/output
          done
        done
      done
    done
