...
for minimum in 5; do # 10 20; do
  ...
  # for model in vgg19; do # do efficientnetB3 vit_large vgg16 desenet201
  # for model in efficientnetB3 vit_large vgg16 vgg19 desenet201; do # do efficientnetB3 vit_large vgg16 desenet201
  for model in desenet201; do # do efficientnetB3 vit_large vgg16 desenet201
    for dataset in pr; do
      for size in 512; do # 400 512; do
        for color in RGB; do # RGB; do
          # for smote in 0 30 60 90 120; do # RGB; do  0 30 60 90 120
          for smote in 60 90 120; do # RGB; do  0 30 60 90 120
            for classifier in MLPClassifier; do # RandomForestClassifier KNeighborsClassifier MLPClassifier SVC; do #DecisionTreeClassifier RandomForestClassifier KNeighborsClassifier MLPClassifier SVC
              python main.py -i /home/msechineli/Documentos/TCC/features/${dataset}+${minimum}/${color}/${size}/${model}/features -s ${smote} -c ${classifier} -o /home/msechineli/Documentos/TCC/smote-grid-cv-fold-parametros-diferentes/${dataset}+${minimum}+${color}+${size}+${model}+${classifier}+SMOTE+${smote}
